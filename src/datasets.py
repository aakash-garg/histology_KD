...
class PatchWiseDataset(Dataset):
    def __init__(self, path, phase, stride=PATCH_SIZE, rotate=False, flip=False, enhance=False, seed = 1):
        ...
        for index in range(len(LABELS)):
            data = natsorted(glob.glob(path + '/' + LABELS[index] + '/*.tif'))
        ...
        self.stride = stride
        if phase == 'train':
            self.labels = training
        elif phase == 'val':
            self.labels = validation
        elif phase == 'test':
            self.labels = testing
        self.names = list(sorted(self.labels.keys()))
        self.shape = (len(self.labels), wp, hp, (4 if rotate else 1), (2 if flip else 1), (2 if enhance else 1))  # (files, x_patches, y_patches, rotations, flip, enhance)
        self.augment_size = np.prod(self.shape) / len(self.labels)
